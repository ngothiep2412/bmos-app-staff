...
import { updateMeal } from "../../redux/actions/mealAction";
...
const UpdateMeal = ({ navigation, route }) => {
  ...
  const [birdMeal, setBirdMeal] = useState();
  ...
  const [statusMeal, setStatusMeal] = useState(false);
  useSetMealProducts(setProducts, isFocused);
  useSetMealBirds(setBirds, isFocused);

  const {
    meal,
    meal: { image, productMeals, status, bird },
  } = useSelector((state) => state.meal);

  useEffect(() => {
    setListProductMealsMorning(productMeals.Morning);

    setListProductMealsAfternoon(productMeals.Afternoon);

    setListProductMealsEvening(productMeals.Evening);

    setImageMeal(image);
    setStatusMeal(status);
    setBirdID(bird.id);
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${server}/bird/${bird.id}`, {
          headers,
        });
    ...
    <KeyboardAvoidingView behavior="padding" style={{ flex: 1 }}>
      ...
      >
        ...
        <View style={{ marginBottom: 20, paddingTop: 70 }}>
          <Text style={formHeading}>Update Meal</Text>
        </View>
            ...
            >
              ...
              >
                <View
                  style={[
                    styles.radioButton,
                    statusMeal === true && styles.radioButtonSelected,
                  ]}
                />
                <Text
                  style={[
                    ...
                    {
                      color:
                        statusMeal === true ? colors.color1 : colors.color2,
                    },
                  ]}
                >
                  True
              ...
              >
                <View
                  style={[
                    styles.radioButton,
                    statusMeal === false && styles.radioButtonSelected,
                  ]}
                />
                <Text
                  style={[
                ...
                <View style={[styles.productContainer, { flex: 2 }]}>
                  ...
                  >
                    ...
                    >
                      ...
                      {birdMeal.birdColor.split(" ").map((item, index) => (
